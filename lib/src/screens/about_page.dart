                ...
                children: const <Widget>[
                  ...
                  _LogEntry(
                    ...
                    items: <String>[
                      ...
                      '修正 Android versionCode 为 2043，补齐 APK 内的 libsqlite3.so，修复启动后白屏的问题。',
                      '自动更新清单和 Android 发布版本升级到 2.3.5+2043。',
                    ],
                  ),
                  SizedBox(height: 14),
                  _LogEntry(
                    ...
                    items: <String>[
                      ...
                      'UI 改进：统一区块标题组件 SectionHeader，搜索结果展示评分，详情页无封面时显示渐变占位。',
                      '交互改进：角色点击统一为底部弹出框（无声优时显示提示），RSS 删除操作增加确认对话框，设置页增加触觉反馈。',
                      '新增首次启动引导页（3 屏介绍核心功能），详情页返回按钮增加 tooltip。',
                  ...
                  _LogEntry(
                    ...
                    items: <String>[
                      ...
                      '修复滚轮监听缺少 hasClients 守卫导致的潜在 crash。',
                      '下载中心 FutureBuilder 增加 error 分支处理，任务标签加载失败时不再静默消失。',
                      '评论空状态文案优化，增加”下拉重试”引导。',
                      '剧集列表等水平滚轮新增 PageStorageKey，切换 Tab 后保持滚动位置。',
                      '在线源搜索错误区分超时/连接失败/其他，各有不同的用户提示。',
                      'Dodo 动漫适配器 HTTP → HTTPS，消除明文传输风险。',
                      '弹幕性能优化：缓存文本宽度计算、跳过冗余 Opacity 包裹。',
                  ...
                  _LogEntry(
                    version: '2.3.2',
                    items: <String>[
                      '工程基础设施：引入 GitHub Actions CI 流水线（lint + test + build），开启 12 条新 lint 规则。',
                      'APK 更新清单新增 SHA256 字段，更新对话框展示校验值供用户核对。',
                      '在线源适配器重构：抽象基类提取共享方法消除约 200 行重复代码，60+ 站点迁移至 JSON 数据驱动。',
                      'API 层新增泛型缓存 helper 消除模板代码，完善类型化异常体系（ServerException + exceptionUserMessage）。',
                      '引入 ServiceLocator 统一管理核心服务注册，降低组件间硬耦合。',
                      '亮色主题适配：PlaybackActionPrompt、下载中心状态色迁移至语义化 colorScheme。',
                      '骨架屏断点与动画网格对齐（480/760/980/1180），消除加载时布局跳变。',
                      '顶部工具栏、动画网格等硬编码中文迁移至 AppStrings 集中管理。',
                      '新增可复用 ErrorStateWidget 错误状态组件（含网络错误/加载失败变体及重试按钮）。',
                      '视频播放器 dispose 链路 14 处静默 catch 增加调试日志输出。',
                      'AnimeCard 增加 Semantics 无障碍标签，删除死代码。',
