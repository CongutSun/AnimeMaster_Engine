# AnimeMaster Engine

<p align="center">
  <img src="assets/icon.png" alt="AnimeMaster" width="128" />
</p>

AnimeMaster Engine 是一个面向动漫追番、资料检索、资源缓存和本地播放的 Flutter 项目。它把 Bangumi 作品资料、收藏进度、磁力/种子资源、本地缓存、播放器和弹幕体验放在同一个客户端里，目标是让用户从“发现一部番”到“看完一集”的路径尽量顺手。

这个仓库既包含可直接运行的 AnimeMaster Android 应用，也保留了 Flutter 插件层、Native FFI 代码、实验性桌面与 iOS 工程，以及可选的 Bangumi OAuth 网关，适合作为个人维护的动漫客户端和工程研究基础。

## 你可以用它做什么

| 场景 | 体验 |
| --- | --- |
| 找番 | 浏览 Bangumi 新番放送、年度排行，通过关键词搜索作品 |
| 看资料 | 查看作品详情、角色、Staff、关联作品和短评 |
| 管收藏 | 登录 Bangumi 后同步收藏、查看进度、更新观看状态 |
| 找资源 | 通过 RSS 聚合源检索磁力或种子资源，并管理下载任务 |
| 做缓存 | 将下载内容沉淀到下载中心，方便后续播放 |
| 看视频 | 使用内置播放器播放本地媒体，支持选集、倍速、全屏和弹幕 |
| 查更新 | 通过应用内更新清单发现新版 APK，并引导下载安装 |

## 核心功能

- **Bangumi 内容中心**：首页新番、排行、搜索、详情、角色、Staff、关联条目和收藏信息。
- **收藏与进度同步**：支持 Bangumi OAuth 登录，读取用户收藏并更新观看进度。
- **资源搜索与下载**：内置磁力/种子聚合搜索、按单集过滤、任务管理、下载中心和基础资源解析能力。
- **本地播放体验**：基于 Flutter 播放页面整合选集、播放控制、亮度/音量、倍速、全屏等常用能力。
- **弹幕与发现**：内置弹弹play 接入，支持文件识别、Bangumi 剧集映射、持久化手动匹配、离线弹幕缓存、热播/飙升榜和新番浏览，以及应用内发送弹幕。
- **应用更新**：支持“检查更新 + 下载 APK”的侧载更新流程，适合个人分发或小范围测试。

## 适合谁

- 想要一个自用追番、资源管理和本地播放客户端的用户。
- 想基于 Flutter 开发动漫内容应用的开发者。
- 想研究 Bangumi API、移动端 OAuth、安全网关、FFI 与播放器整合的项目维护者。

## 当前状态

AnimeMaster Engine 仍处在持续迭代阶段，功能已经能串起主要使用流程，但不是一个完全打磨完成的商店级应用。

- Android 是当前正式支持和持续构建验证的平台。
- iOS 与 Windows 工程属于实验性支持；使用前需要在对应平台完成构建和设备回归。
- Bangumi 安全登录需要配合授权网关，客户端不会保存 `App Secret`。
- 弹弹play 默认通过自有网关接入，无需用户填写密钥；自定义凭据入口保留在高级设置中。发送额度全应用共享（每日 10 条、每月 240 条），发送前显示剩余额度。弹弹play 账号、关注与历史同步尚未获得开放权限，不提供登录入口。
- 项目中仍有少量历史中文文案编码问题，后续可以继续清理。

## 安全、隐私与内容边界

- Android 应用更新会在安装前验证 HTTPS、SHA-256、包名、版本号和当前应用签名。
- Bangumi 令牌和弹弹play密钥保存在系统安全存储中，不写入普通偏好设置。
- 在线源、RSS、视频、弹幕和种子均来自第三方服务；请遵守所在地法律和对应服务条款。
- 项目不托管第三方媒体内容，也不保证外部来源的可用性、合法性和安全性。
- 详细说明见 [PRIVACY.md](PRIVACY.md)、[LEGAL.md](LEGAL.md) 和 [SECURITY.md](SECURITY.md)。

## 快速体验

示例应用位于 `example/` 目录。第一次运行前请确保本机已经安装 Flutter、Dart、目标平台 SDK 和对应设备环境。

```powershell
git clone https://github.com/CongutSun/AnimeMaster_Engine.git
cd AnimeMaster_Engine\example
flutter pub get
flutter run
```

如果你有多个设备，可以先查看设备列表，再指定目标设备：

```powershell
flutter devices
flutter run -d <deviceId>
```

## 功能入口

| 页面/模块 | 说明 |
| --- | --- |
| 首页 | 展示新番放送、年度排行和内容入口 |
| 搜索 | 检索 Bangumi 条目并进入详情 |
| 详情页 | 查看作品信息、角色、Staff、关联作品和可用资源 |
| 收藏页 | 展示用户收藏与观看进度 |
| 下载中心 | 管理资源下载与缓存任务 |
| 播放器 | 播放本地视频，加载弹幕和常用播放控制 |
| 设置页 | 配置 Bangumi 登录、弹幕密钥、更新地址等应用选项 |

## 可选配置

普通体验不一定需要一次性配置所有服务。按你的使用场景逐步打开即可。

- **Bangumi 登录**：用于同步收藏、进度和用户信息。正式链路通过 HTTPS 授权网关完成，避免在客户端保存密钥。
- **弹弹play 弹幕**：默认使用内置网关，无需填写凭据。拥有个人应用凭据的用户可在设置的“自定义接入”中覆盖默认配置。弹幕与发现数据由[弹弹play开放弹幕网络](https://www.dandanplay.com/)提供。
- **安全更新**：在设置页填写 HTTPS 更新清单 URL 后，Android 会下载并校验 SHA-256、包名、版本号和签名，再交由系统安装。
- **RSS 资源源**：用于搜索磁力和 `.torrent` 种子并创建下载任务，具体可用性取决于你配置的源和网络环境。

### RSS 搜索与追番

在设置的“RSS 资源源”中添加网站提供的 HTTPS 地址。“搜索源”使用含 `{keyword}` 的地址；“订阅源”使用固定地址，兼容 RSS 和 Atom。保存前可测试连接，查看条目数、可下载数量和最新发布时间。个人地址保存在设备安全存储中，列表只显示域名；自定义来源不经过公共代理。

首页与“我的二次元库”中的“订阅更新”集中展示资源。登录 Bangumi 后，资源会按在看番剧匹配；可设置别名、字幕组、画质、排除词、来源和集数偏移。合集、特别篇及集数不明的内容进入待确认，支持手动关联作品。下载需要确认，取消不会标记为已下载；忽略和已下载状态保存在本机。

应用启动、回到前台或手动刷新时检查订阅，自动检查间隔至少 30 分钟。本版本不在应用关闭后定时检查，也不会自动下载或修改观看进度。固定订阅搜索仅覆盖源当前提供的条目，不代表网站完整历史库；更新列表最多保留 1,000 条资源。

## 技术组成

- Flutter + Dart：主要界面、业务流程和跨平台应用层。
- Provider：应用设置、下载状态等轻量状态管理。
- Dio / html / dart_rss：网络请求、页面解析和 RSS 解析。
- media_kit：本地媒体播放能力。
- FFI + Native C/C++：保留底层原生扩展能力。
- Cloudflare Workers 或 Node.js：可选的 Bangumi OAuth 授权网关示例。

## 仓库结构

```text
lib/                         Flutter 业务代码与应用页面
src/                         Native FFI 代码
example/                     可运行、调试和打包的示例应用
example/RELEASE.md           Android 发版与更新清单说明
tools/bangumi_auth_worker/   Cloudflare Workers 版 Bangumi OAuth 网关
tools/bangumi_auth_gateway/  Node.js 版 Bangumi OAuth 网关
assets/                      应用图标等静态资源
release/                     更新清单示例
```

## 开发者入口

根目录用于插件和共享代码，实际应用运行入口在 `example/`。

```powershell
cd AnimeMaster_Engine
flutter pub get

cd example
flutter pub get
flutter analyze
```

构建 Android APK：

```powershell
cd example
flutter build apk --release
```

更完整的 Android 发版流程请看 `example/RELEASE.md`。Bangumi OAuth 网关代码位于 `tools/` 目录，适合维护者按自己的域名、Secret 和部署平台进行配置。

## 说明

AnimeMaster Engine 更偏向个人使用和持续演进中的客户端工程，而不是一个只需部署即可上线的服务端项目。README 的重点是帮助用户理解它能解决什么问题；具体网关部署、签名打包和更新分发可以在需要维护发布版本时再进入对应目录查看。
